...
float cotaSupOpt(vector<int> const& objetos, DatosProblema const& dp, float b, float tam_usado, int indice) {
	float res = b;
	float tam_disponible = dp.tam_Mochila - tam_usado;
	while (indice < objetos.size() && dp.pesos[objetos[indice]] <= tam_disponible) {
		tam_disponible -= dp.pesos[objetos[indice]];
		res += dp.valores[objetos[indice]];
		indice++;
	}
	if (indice < objetos.size()) {
		res += (tam_disponible / dp.pesos[objetos[indice]] * dp.valores[objetos[indice]]);
	}
...
void VueltaAtrasOptimalidad(vector<int>const & objetos, DatosProblema const& dp, float valor_candidato, float peso_candidato, float& sol, int indiceElemento, size_t & nodos) {
